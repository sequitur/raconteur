...
Function.prototype.inherits = function(Parent) {
    const d = {}, p = (this.prototype = new Parent());
...
const isMobileDevice = function() {
    /*
    We had two different things checking for the size of the
    viewport: CSS media queries, and javascript.

    Unfortunately, it is impossible to match them in a reliable,
    cross-browser way. So instead we delegate authority to the
    CSS media query, by looking at a magical property on a
    nonexistant entity.

    FIXME; this is hacky and it causes severe jank when
    resizing the viewport, but it's the most reliable method
    so far.
    */

    return $('.mobilecheck').css('float') === 'right';
};
...
const Situation = function(opts) {
    if (opts) {
        ...
        if (opts.tags !== undefined) {
            if ($.isArray(opts.tags)) this.tags = opts.tags;
            else this.tags = opts.tags.split(/[ \t,]+/);
        } else {
            this.tags = [];
        }
    } else {
...
Situation.prototype.canView = function(character, system, situation) {
    if ($.isFunction(this._canView)) return this._canView(character, system, situation);
    else return this._canView;
};
...
Situation.prototype.choiceData = function() {
    return {
 ...
 * the base Situation.
 */
const SimpleSituation = function(content, opts) {
...
SimpleSituation.prototype.enter = function(character, system, from) {
    ...
    if (this.choices) {
        const choices = system.getSituationIdChoices(this.choices,
                                                     this.minChoices,
                                                     this.maxChoices);
...
SimpleSituation.prototype.act = function(character, system, action) {
    const response = this.actions[action];
    try {
...
SimpleSituation.prototype.optionText = function(character, system, sitn) {
    const parentResult = Situation.prototype.optionText.call(this, character,
                                                           system, sitn);
    if (parentResult === undefined) return this.heading;
 ...
 */
const QualityDefinition = function(title, opts) {
    const myOpts = $.extend({
        priority: title,
        group: null,
        extraClasses: null
...
const IntegerQuality = function(title, opts) {
...
const NonZeroIntegerQuality = function(title, opts) {
 ...
 * QualityDefinition. */
const NumericQuality = function(title, opts) {
    QualityDefinition.call(this, title, opts);
};
 ...
 */
const WordScaleQuality = function(title, values, opts) {
    const myOpts = $.extend({
        offset: null,
        useBonuses: true
    }, opts);
...
WordScaleQuality.prototype.format = function(character, value) {
    let val = Math.floor(value - this.offset);
    let mod = "";
    if (val < 0) {
        mod = val.toString();
        val = 0;
 ...
 */
const FudgeAdjectivesQuality = function(title, opts) {
...
const OnOffQuality = function(title, opts) {
    const myOpts = $.extend({
        onDisplay: ""
    }, opts);
    QualityDefinition.call(this, title, opts);
 ...
 */
const YesNoQuality = function(title, opts) {
    const myOpts = $.extend({
        yesDisplay: "yes".l(),
        noDisplay: "no".l()
    }, opts);
...
const QualityGroup = function(title, opts) {
    const myOpts = $.extend({
        priority: title,
        extraClasses: null
    }, opts);
...
System.prototype.writeHeading = function(headingContent, elementSelector) {
    const heading = $("<h1>").html(headingContent);
...
System.prototype.writeChoices = function(listOfIds, elementSelector) {
    ...
    const $options = $("<ul>").addClass("options");
    for (let i = 0; i < listOfIds.length; ++i) {
        const situationId = listOfIds[i];
        const situation = game.situations[situationId];
        assert(situation, "unknown_situation".l({id:situationId}));

        let optionText = situation.optionText(character, this,
                                              currentSituation);
...
System.prototype.getSituationIdChoices = function(listOfOrOneIdsOrTags,
    ...
    const allIds = {};
    for (let i = 0; i < listOfOrOneIdsOrTags.length; ++i) {
        const tagOrId = listOfOrOneIdsOrTags[i];
        if (tagOrId.substr(0, 1) === '#') {
            let ids = getSituationIdsWithTag(tagOrId.substr(1));
    ...
    const currentSituation = getCurrentSituation();
    const viewableSituationData = [];
    for (let situationId in allIds) {
        let situation = game.situations[situationId];
        assert(situation, "unknown_situation".l({id:situationId}));

        if (situation.canView(character, system, currentSituation)) {
            // While we're here, get the selection data.
            const viewableSituationDatum =
    ...
    const committed = [];
    let candidatesAtLastPriority = [];
    let lastPriority;
    // In descending priority order.
    for (let i = 0; i < viewableSituationData.length; ++i) {
    ...
    const totalChoices = committed.length + candidatesAtLastPriority.length;
    if (maxChoices === undefined || maxChoices >= totalChoices) {
        ...
        const candidatesToInclude = maxChoices - committed.length;
        for (let i = 0; i < candidatesAtLastPriority.length; ++i) {
        ...
        const chosen = candidatesToInclude.slice(0, candidatesToInclude);
...
System.prototype.setCharacterText = function(content) {
    const block = $("#character_text_content");
    // FIXME: This line used to be here, but the variable is never used. ???
    // var oldContent = block.html();
    const newContent = augmentLinks(content);
...
System.prototype.setQuality = function(quality, newValue) {
    const oldValue = character.qualities[quality];
    character.qualities[quality] = newValue;
    if (!interactive) return;

    // Work out how to display the values.
    let newDisplay;
    const qualityDefinition = game.qualities[quality];
    ...
    if (qualityBlock.length <= 0) {
        ...
        if (newDisplay === null) {
            ...
            let toRemove = null;
            let groupBlock = qualityBlock.parents('.quality_group');
            if (groupBlock.find('.quality').length <= 1)
                toRemove = groupBlock;
            else
                toRemove = qualityBlock;

            toRemove.fadeOut(1000, () => {
                toRemove.remove();
            });
        } else {
            let valBlock = qualityBlock.find("[data-attr='value']");
            valBlock.fadeOut(250, () => {
...
System.prototype.animateQuality = function(quality, newValue, opts) {
    let currentValue = character.qualities[quality];
    ...
    const bar = $("#ui_library #progress_bar").clone();
    bar.removeAttr("id");
    const widthElement = bar.find("[data-attr='width']");
    widthElement.css('width', myOpts.from*totalWidth);

    // Configure its labels
    const titleLabel = bar.find("[data-attr='name']");
    const valueLabel = bar.find("[data-attr='value']");
    const leftLabel = bar.find("[data-attr='left_label']");
    const rightLabel = bar.find("[data-attr='right_label']");
    if (myOpts.title)
...
const getCurrentSituation = function() {
    if (current)
...
const parseList = function(str, canBeUndefined) {
    if (str === undefined) {
        if (canBeUndefined)
            return undefined;
        else
...
const parseFn = function(str) {
    if (str === undefined) {
        ...
        const fstr = `(function(character, system, situation) {\n${str}\n})`;
        const fn = eval(fstr);
        return fn;
    }
};

const loadHTMLSituations = function() {
    const $htmlSituations = $("div.situation");
    $htmlSituations.each(function() {
        const $situation = $(this);
        const id = $situation.attr("id");
        assert(game.situations[id] === undefined,
               "existing_situation".l({id:id}));

        const content = $situation.html();
        const opts = {
            // Situation content
 ...
 */
const doWrite = function(content, selector, addMethod, appendMethod) {
    const output = augmentLinks(content).addClass('new');
    let element;
    if (selector) element = $(selector);

    if (!element) $('#content')[addMethod](output);
    else element[appendMethod](output);
};

/* Gets the unique id used to identify saved games. */
const getSaveId = function() {
    return `undum_${game.id}_${game.version}`;
};

/* Adds the quality blocks to the character tools. */
const showQualities = function() {
    $("#qualities").empty();
    for (let qualityId in character.qualities)
        addQualityBlock(qualityId);
};

/* Fades in and out a highlight on the given element. */
const showHighlight = function(domElement) {
    let highlight = domElement.find(".highlight");
...
const insertAtCorrectPosition = function(parent, newItem) {
    const newPriority = newItem.attr('data-priority');
...
const addGroupBlock = function(groupId) {
    const groupDefinition = game.qualityGroups[groupId];

    // Build the group div with appropriate heading.
    const groupBlock = $("#ui_library #quality_group").clone();
    groupBlock.attr("id", `g_${groupId}`);
    groupBlock.attr("data-priority", groupDefinition.priority);

    const titleElement = groupBlock.find("[data-attr='title']");
    ...
    if (groupDefinition.extraClasses) {
        for (let i = 0; i < groupDefinition.extraClasses.length; i++)
...
const addQualityBlock = function(qualityId) {
    // Make sure we want to display this quality.
    const qualityDefinition = game.qualities[qualityId];
    if (!qualityDefinition) {
      throw new Error(
        `Can't display a quality that hasn't been defined: ${qualityId}`);
    }

    // Work out how the value should be displayed.
    const name = qualityDefinition.title;
    const val = qualityDefinition.format(
        character, character.qualities[qualityId]
    );
    if (val === null) return null;

    // Create the quality output.
    const qualityBlock = $("#ui_library #quality").clone();
    ...
    if (qualityDefinition.extraClasses) {
        for (let i = 0; i < qualityDefinition.extraClasses.length; i++)
            qualityBlock.addClass(qualityDefinition.extraClasses[i]);
    }

    // Find or create the group block.
    let groupBlock;
    let groupId = qualityDefinition.group;
    if (groupId) {
        let group = game.qualityGroups[groupId];
        assert(group, "no_group_definition".l({id: groupId}));
        groupBlock = $(`#g_${groupId}`);
        if (groupBlock.length <= 0) groupBlock = addGroupBlock(groupId);
    }

    // Position it correctly.
    const groupQualityList = groupBlock.find(".qualities_in_group");
...
const endOutputTransaction = function () {
    if (!interactive) return; // We're loading a save; do nothing at all.
    const
        $new = $('.new'),
        viewHeight = $(window).height();
    let newTop, newBottom, newHeight, optionHeight = 0;
...
const linkRe = /^([a-z0-9_-]+|\.)(\/([0-9a-z_-]+))?$/;
const processLink = function(code) {
 ...
 */
const processOneLink = function(code) {
    const match = code.match(linkRe);
    assert(match, "link_not_valid".l({link:code}));

    let situation = match[1];
    ...
    if (action) {
        ...
        if (situation) {
            if (game.beforeAction) {
                ...
                const consumed = game.beforeAction(
                    character, system, current, action
                );
                if (consumed !== true) situation.act(character, system, action);
            } else {
 ...
 * action. */
const processClick = function(code) {
...
const doTransitionTo = function(newSituationId) {
    const oldSituationId = current;
    const oldSituation = getCurrentSituation();
    const newSituation = game.situations[newSituationId];
    ...
    if (oldSituation) {
        ...
        let contentToHide = $('#content .transient, #content ul.options');
 ...
 * wired up. */
const augmentLinks = function(content) {
...
const getSituationIdsWithTag = function(tag) {
    const result = [];
    for (let situationId in game.situations) {
        let situation = game.situations[situationId];

        for (let i = 0; i < situation.tags.length; ++i) {
 ...
 */
const initGameDisplay = function() { // eslint-disable-line no-unused-vars
...
const loadGame = function(characterData) {
    ...
    interactive = false;
    for (let i = 0; i < progress.sequence.length; i++) {
        let step = progress.sequence[i];
...
/* Set up the game when everything is loaded. */
    ...
    // Handle storage.
    if (hasLocalStorage()) {
        const erase = $("#erase").click(() => {
            doErase();
        });
        const save = $("#save").click(saveGame);

        const storedCharacter = localStorage[getSaveId()];
        if (storedCharacter) {
            try {
    ...
    // first links.
    $("body").on('click', "ul.options li, #menu li", () => {
    ...
    const resize = function() {
        ...
        if (wasMobile !== mobile) {
            const showing = !$(".click_message").is(":visible");
            if (mobile) {
                const menu = $("#menu");
                if (showing) {
                    $("#toolbar").show();
